...
let splitStringOnCommas (x :string) =
    x.Split([|','|])

let openCloseSpread (dayQuote : string[]) =
    abs(float dayQuote.[1] - float dayQuote.[4])

let getQuoteDate (dayQuote : string[]) =
    dayQuote.[0]

let dayWithGreatestVariance = 
    stockData
    |> List.map splitStringOnCommas
    |> List.maxBy openCloseSpread
    |> 
